...
func (g *Grafana) getOrgId(orgname string) (int, error){
	org, err := g.GetOrgByName(orgname)
	id := org.Id
	return id, err
}
...
func (g *Grafana) GetOrgMember(org interface{}) ([]User, error){
  ...
  switch org.(type) {
    case string:
      id, _ = g.getOrgId(org.(string))
    case int:
      id = int(org.(int))
    default:
      id = 0
  }
  url := fmt.Sprintf("/api/orgs/%d/members",id)
  req, err := g.newRequest("GET",url,nil,nil)
  if err != nil {
    return users, err
  }
...
func (g *Grafana) AddOrgMember(user, org string) error {
  u,_ := g.GetUser(user)
  uid := u.Id
  tid, _ := g.getOrgId(org)
  url := fmt.Sprintf("/api/orgs/%d/members",tid)
...
func (g *Grafana) RemoveOrgMember(user, org string) error {
  u,_ := g.GetUser(user)
  uid := u.Id
  tid, _ := g.getOrgId(org)
